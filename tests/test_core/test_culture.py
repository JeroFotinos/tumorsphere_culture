...
def test_early_stage_cell_number(evolved_culture, expected_number_of_cells, request):
...
def test_deactivation_small():
    ...
    try:
        ...
        # we evolve it
        simulation_steps = 6
        deactivated_culture.simulate(num_times=simulation_steps, culture_name=culture_name_temp)
        ...
        # we assert that all cells were deactivated
        assert all([cell.available_space is False for cell in deactivated_culture.cells])
        ...
        # we assert that it's located at the origin
        assert first_cell.culture.cell_positions[first_cell._index].all() == np.array([0, 0, 0]).all()
        ...
        deactivated_culture.reproduce(cell_index=first_cell._index, tic=6, dat_files=False)
        # the fixture had simulation_steps = 6, so the last tic was 5 (the loop in
        # simulate is: for i in range(1, num_times) ...)
...
def test_deactivation_big():
    ...
    try:
        ...
        assert len(deactivated_culture.active_cell_indexes) < len(deactivated_culture.cells)
        assert len(deactivated_culture.active_cell_indexes) > 0
