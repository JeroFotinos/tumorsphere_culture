...
def test_cli_simulate(run_cli, compare_dbs_ignore_timestamp):
    ...
    try:
        ...
        command = (
            f"tumorsphere simulate --prob-stem 0.4,0.8 --prob-diff 0 "
            f"--realizations 2 --steps-per-realization 7 --rng-seed 1234 "
            f"--sql True --dat-files True --ovito True "
        ...
        # Compare the contents of the output and expected directories
        ...
        assert output_files == expected_files, (
            "Actual and expected output directories "
            "do not have the same set of files."
        )

        # Compare the content of each file, using the appropriate method for
        # .db files
        for file_name in output_files:
            if file_name.endswith(".db"):
                # Use the database comparison fixture for .db files
                assert compare_dbs_ignore_timestamp(
                    output_dir / file_name, expected_dir / file_name
                ), f"Database contents do not match: {file_name}"
            else:
                # Use filecmp for non-database files
                assert filecmp.cmp(
...
def test_cli_mergedbs(run_cli):
    ...
    correct_merged_db = (
        cwd / "data/testing_mergedbs_cli/expected_result/merged.db"
    )
    ...
    ), "The merged databases are not equal"
