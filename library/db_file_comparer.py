...
def compare_databases(db_path1, db_path2):
    ...
    schema1 = conn1.execute("SELECT name, sql FROM sqlite_master WHERE type='table' ORDER BY name").fetchall()
    schema2 = conn2.execute("SELECT name, sql FROM sqlite_master WHERE type='table' ORDER BY name").fetchall()
    ...
    for table_name, _ in schema1:
        rows1 = conn1.execute(f"SELECT * FROM {table_name} ORDER BY rowid").fetchall()
        rows2 = conn2.execute(f"SELECT * FROM {table_name} ORDER BY rowid").fetchall()
