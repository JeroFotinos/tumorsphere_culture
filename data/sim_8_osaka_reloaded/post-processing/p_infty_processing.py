    ...
    for p_d in df["pd"].unique():
        for ps in df[df["pd"] == p_d]["ps"].unique():
            for culture_seed in df[(df["pd"] == p_d) & (df["ps"] == ps)][
                ...
                new_rows["culture_seed"].append(culture_seed)
...
def add_active_stem_cells_indicator(df):
    """
    Adds a new column to the DataFrame that indicates whether there are active
    stem cells.
...
def average_over_realizations(df):
    ...
    We average the dataframe over the realizations (i.e., over the culture
    seed), gruping by pd, pd and the time step.
...
def plot_fitted_pc_vs_t(
    ...
    for pd_value in pd_values:
        ...
        ).to_csv(f"{csv_output_path}_pd_{pd_value}.csv", index=False)
        ...
        plt.savefig(f"{png_output_path}_pd_{pd_value}.png", dpi=600)
        plt.close(fig)  # Close the figure to free up memory
...
if __name__ == "__main__":
    csv_file = "/home/nate/Devel/tumorsphere_culture/data/sim_7_short_time_broad_ps/csv_files/df_simulations.csv"
    plot1_output_path = "/home/nate/Devel/tumorsphere_culture/data/sim_7_short_time_broad_ps/results-images/p_infty_vs_ps"
    csv_output_path = "/home/nate/Devel/tumorsphere_culture/data/sim_7_short_time_broad_ps/csv_files/"
    plot2_output_path = "/home/nate/Devel/tumorsphere_culture/data/sim_7_short_time_broad_ps/results-images/pc_vs_t"
    png_output_path = "/home/nate/Devel/tumorsphere_culture/data/sim_7_short_time_broad_ps/results-images/"
