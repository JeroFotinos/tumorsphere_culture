...
# Compute the differences between the differences (second derivative)
time_dif_of_dif = np.zeros(len(time_dif) - 1)
for n in range(1, len(time_dif)):
    time_dif_of_dif[n - 1] = time_dif[n] - time_dif[n - 1]


# Create a plot with the time values on the y-axis and the step number on the x-axis
steps = range(1, len(time_dif))  # Generate a list of step numbers (1-indexed)
plt.plot(steps, time_dif_of_dif, marker=".")
...
xlabel = r"$n$"
ylabel = r"$\Delta t(n) - \Delta t(n-1)$ [minutes]"
title = r"Difference in time increments in calculating the next step (second derivative)"
...
plt.savefig(
    "./data/sim_1_time_measurement/post_processing/time_second_derivative.png",
    dpi=600,
)
